...
/*	public function _initialize() {
	...
	*/
	...
	{
		$node = D('Node');
		$nodeData = getTree($node->field('id,name,remark,title,pid,status')->select());
		$count = $node->count();
	...
	public function admin_permission_add()
	{
		...
		if(IS_POST){
			...
			if(!$data = $node->create()){
				...
				if($node->add()){
					$this->success('添加成功', U('Admin/Node/admin_permission'), 2);

				}else{
					$this->error('添加失败', U('Admin/Node/admin_permission_add'), 2);
				}
			}
		}else{
			$nodeData = getTree($node->field('id,title,pid,status')->where('status=1')->select());
	...
	public function admin_permission_edit()
	{
		...
		if(IS_POST){
			if(!$data = $node->create()){
				...
				if($node->save($data)){
					$this->success('修改成功', U('Admin/Node/admin_permission'), 2);
				}else{
					$this->error('修改失败', U('Admin/Node/admin_permission'), 2);
				}
			...
			$id = I('get.id');
			$data = getTree($node->field('id,title,pid,status')->where('status=1')->select());
			$nodeData = $node->field('id,name,title,pid,remark,sort,level')->find($id);
			$this->assign('nodeData', $nodeData);
	...
	public function admin_permission_del()
	{
		...
		$check = $node->field('pid')->where('pid='.$id)->select();
		if($check){
			echo json_encode('有下级节点,不能直接删除');
		}else{
			if($node->delete($id)){
